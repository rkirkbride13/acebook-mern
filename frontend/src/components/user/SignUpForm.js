    ...
    fetch("/users", {
      ...
      if (response.status === 201) {
        ...
        setUserExsistsErrorMessage(
          "That user already exists - Please create a new account or login"
        );
      } else {
        navigate("/signup");
  ...
  return (
    <>
      <div className="titlecenter">
        ...
        <a className="content" href="../login">
          login here
        </a>
      </div>
      ...
      <div className="signupform">
        ...
        <div className="errorMessages">
          {UserExsistsErrorMessage && (
            <p className="error"> {UserExsistsErrorMessage} </p>
          )}
          {EmptyFieldErrorMessage && (
            <p className="error"> {EmptyFieldErrorMessage} </p>
          )}
          {GeneralErrorMessage && (
            <p className="error"> {GeneralErrorMessage} </p>
          )}
        </div>
